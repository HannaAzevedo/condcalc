...
import { DollarSign, Droplets, Home, PieChart, Users, Thermometer } from "lucide-react"; 
import {
  Tooltip,
  TooltipContent,
  TooltipProvider,
...
export function ResultsTable({ calculatedData, commonExpenses, tariffRates }: ResultsTableProps) {
  ...
  const totalPreviousReading = calculatedData.reduce((sum, ap) => sum + ap.previousReading, 0);
  const totalCurrentReading = calculatedData.reduce((sum, ap) => sum + ap.currentReading, 0);
  ...
  const totalExcessTierCostTotal = calculatedData.reduce((sum, ap) => sum + ap.excessTierCostTotal, 0); 
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right font-semibold text-foreground"><Thermometer className="inline mr-1 h-4 w-4" />Leitura Anterior (m³)</TableHead>
                <TableHead className="text-right font-semibold text-foreground"><Thermometer className="inline mr-1 h-4 w-4" />Leitura Atual (m³)</TableHead>
            ...
            <TableBody>
              {calculatedData.map((ap) => (
                <TableRow key={ap.id} className="hover:bg-secondary/30">
                  ...
                  <TableCell className="text-right">{Math.round(ap.previousReading)}</TableCell>
                  <TableCell className="text-right">{Math.round(ap.currentReading)}</TableCell>
                  <TableCell className="text-right">{Math.round(ap.consumption)}</TableCell>
                      ...
                      <Tooltip>
                        ...
                        <TooltipContent className="max-w-xs bg-popover text-popover-foreground border shadow-md p-3 rounded-lg text-xs">
                          <div className="space-y-1">
                            ...
                            {Object.entries(ap.tierCostBreakdown || {}).map(([tierName, tierCost]) => {
                                if (tierName !== fixedTierNameFromConfig && tierName !== firstExceedingTierFromConfig?.name && tierName !== "Ajuste Diferença Fatura") {
                                    return (
                                        <div className="flex justify-between" key={tierName}>
                                            <span>{tierName}:</span>
                                            <span>{formatCurrency(tierCost)}</span>
                                        </div>
                                    );
                                }
                                return null;
                            })}
                            {ap.tierCostBreakdown?.["Ajuste Diferença Fatura"] !== undefined && (
                                <div className="flex justify-between pt-1 border-t mt-1">
                                    <span className="font-medium">Ajuste Fatura Concessionária:</span>
                                    <span>{formatCurrency(ap.tierCostBreakdown["Ajuste Diferença Fatura"])}</span>
                                </div>
                            )}
            ...
            <TableFooter>
              <TableRow className="font-semibold bg-muted/80 text-sm">
                ...
                <TableCell className="text-right"></TableCell>{/* Leitura Anterior - Sem total */}
                <TableCell className="text-right"></TableCell>{/* Leitura Atual - Sem total */}
                <TableCell className="text-right">{Math.round(totalConsumption)} m³</TableCell>
                <TableCell className="text-right">{formatCurrency(totalMinimumFixedCostShare)}</TableCell>
                <TableCell className="text-right">{formatCurrency(totalExcessTierCostTotal)}</TableCell>
